...
void Player::Attack()
{
	...
	if (_deltaTime > _fireSpeed)
	{
		...
		else
		{
			// bone 006
			// bone 010
			Vec3 pos1 = GetGameObject()->GetTransform()->GetChild(0)->GetChild(static_cast<int>(_isCombatMode))->GetAnimator()->GetBonePosition("bone006");
			Vec3 pos2 = GetGameObject()->GetTransform()->GetChild(0)->GetChild(static_cast<int>(_isCombatMode))->GetAnimator()->GetBonePosition("bone010");
